...
def main_menu_keyboard(is_admin: bool = False, is_premium: bool = False):
    """Main menu keyboard - alias for get_main_menu for compatibility"""
    return get_main_menu(is_admin, is_premium)

def premium_keyboard():
    """Premium keyboard - returns premium purchase options"""
    buttons = [
        [
...
def get_conversation_keyboard(language):
    ...
    if language == "korean":
        buttons = [
            ...
            [
                InlineKeyboardButton(text="🔙 Suhbat menu", callback_data="ai_conversation"),
                InlineKeyboardButton(text="🏠 Bosh menu", callback_data="main_menu")
            ]
        ]
    else:  # japanese
