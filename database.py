...
async def get_user_stats(user_id: int) -> Optional[Tuple[Any, ...]]:
    """Get user statistics - returns tuple for compatibility"""
    async with aiosqlite.connect(DATABASE_PATH) as db:
        cursor = await db.execute("""
            SELECT user_id, rating_score, total_sessions, words_learned, 
                   is_premium, referral_count, premium_expires_at
            FROM users WHERE user_id = ?
        """, (user_id,))
        result = await cursor.fetchone()
        
        if not result:
            # Return default values in tuple format
            return (user_id, 0.0, 0, 0, False, 0, None)
